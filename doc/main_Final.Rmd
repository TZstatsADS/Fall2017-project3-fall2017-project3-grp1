...
model_values <- seq(4, 8, 2) # for GBM
...
# DO: train selected models
model <- train( dat_train, label_train, run.gbm = TRUE )
...
if(run.cv){
  ...
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv[k,] <- cv.function( dat_train, label_train, model_values[k], K, cv.lda = T)
  }
